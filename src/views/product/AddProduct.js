...
const AddProduct = () => {
  ...
  const saveAdvantages = () => {
    ...
    axios
      .post(
        `http://markbran.in/apis/admin/product/advantages/`,
        advantagesData,
        {
          headers: {
            "auth-token": jwtToken, //the token is a variable which holds the token
          ...
          <CCardBody>
            <ul className="nav">
              <li className="nav-item">
                <button
                  onClick={() => setPage(1)}
                  className={`nav-link active btn btn-${
