...
const Material = () => {
    ...
    // 
    const clickOnDelete = (materialId) => {
        confirmAlert({
            title: 'Are you sure?',
    ...
    const deleteMaterial = (materialId) => {
        // alert(materialId)
        axios.delete(`http://markbran.in/apis/admin/material/${materialId}`, {
            headers: {
    ...
    const axiosCategories = () => {
        axios.get('http://markbran.in/apis/admin/material', {
            headers: {
                "auth-token": jwtToken //the token is a variable which holds the token
            }
        })
        ...
        <CRow>
            <CCol xl={12}>
                <CCard>
                    <CCardHeader>
                        Material
                        <CLink style={{ float: 'right' }} className="btn btn-success" to="/material/add">
                            Add Material
                        </CLink>
                    </CCardHeader>
                    <CCardBody>
                            ...
                            <tbody>
                                {materials.map((item, index) =>
                                    <tr key={item.id}>
                                        ...
                                        <td>{item.title}</td>
                                        {/* <td>{item.title}</td> */}
                                        <td><img className="img-responsive w-100px" src={process.env.REACT_APP_BASE_URL + item.image} alt="" /></td>
                                        <td>{dateFormat(item.createdAt, "mmmm dS, yyyy")}</td>
                                        <td>
                                            <CLink className="btn btn-sm btn-outline-warning" to={`/material/edit/${item.id}`}>
                                                Edit
                                            </CLink>
